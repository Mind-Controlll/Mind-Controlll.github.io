...
GENERATED_DIR = CONTENT_DIR / "_generated"
RECENT_POSTS_FILE = GENERATED_DIR / "recent-posts.typ"
...
def _unescape_typst_string(value: str) -> str:
    """Unescape the small subset of Typst string escapes used in metadata."""
    return (
        value.replace(r"\\", "\\")
        .replace(r"\"", '"')
        .replace(r"\n", "\n")
...
def _escape_typst_string(value: str) -> str:
    """Escape text so it can be written as a Typst string literal."""
    return (
        value.replace("\\", r"\\")
        .replace('"', r"\"")
        .replace("\n", r"\n")
...
def _extract_typst_string_arg(content: str, name: str) -> str:
    match = re.search(rf'{name}\s*:\s*"((?:\\.|[^"\\])*)"', content, re.DOTALL)
    if not match:
        return ""
    return _unescape_typst_string(match.group(1)).strip()


def _extract_typst_datetime_arg(content: str, name: str) -> datetime | None:
    match = re.search(
        rf"{name}\s*:\s*datetime\(\s*year:\s*(\d{{4}})\s*,\s*month:\s*(\d{{1,2}})\s*,\s*day:\s*(\d{{1,2}})",
        content,
        re.DOTALL,
    )
    if not match:
...
def collect_source_posts() -> list[dict]:
    ...
    posts = []
    for typ_file in blog_dir.rglob("index.typ"):
        if typ_file == blog_dir / "index.typ":
            continue

        try:
            content = typ_file.read_text(encoding="utf-8")
        except Exception as e:
            print(f"⚠️ 无法读取文章元数据 {typ_file}: {e}")
            continue

        date_obj = _extract_typst_datetime_arg(content, "date")
        if not date_obj:
            continue
...
def generate_recent_posts_data() -> bool:
    """Generate a Typst data file used by the homepage latest-post cards."""
    try:
        posts = collect_source_posts()
        ...
        for post in posts:
            ...
            lines.extend(
                [
                    "  (",
                    f'    title: "{_escape_typst_string(post["title"])}",',
...
def build_html(force: bool = False) -> bool:
    ...
    if not generate_recent_posts_data():
        return False
...
def collect_posts(dirs: set[str], site_url: str) -> list[dict]:
    ...
    for d in dirs:
        ...
        for index_html in dir_path.rglob("index.html"):
            if index_html == dir_path / "index.html":
                continue

            item = index_html.parent
