...
这个博客使用 GitHub Actions 部署。你只要把代码推送到 `main` 分支，GitHub 就会自动运行 `.github/workflows/pages.yml`，构建并发布网站。
...
推送完成后去 GitHub 的 Actions 页面，看到 `Deploy GitHub Pages` 变成绿色对勾，就说明部署成功。

== 常见问题

- 如果网页还是旧内容，先按 `Ctrl + F5` 强制刷新。
- 如果 Actions 没有运行，检查工作流文件是否在 `.github/workflows/pages.yml`。
